...
void init_iba() {
    ...
    scale = (float)GetScreenHeight() / bg_image_iba.height;
    float y_pos_floor = (float)GetScreenHeight() - scale * 350;
   // playerPos_iba = (Vector2){-10, y_pos_floor};
    playerPos_iba = {2200*scale,GetScreenHeight()-400*scale};
    exit_zone_iba = {2200*scale,GetScreenHeight()-400*scale};
    pop_up_zone = {3250*scale,GetScreenHeight()-540*scale};
    game_zone_iba ={ 2400*scale,GetScreenHeight()-400*scale};                                     
...
void logic_draw_iba() {
    ...
    if(iba_game && bank_game) 
    {
        DrawText("IBA Conquered", 20, screenHeight - 90, 20, GREEN);
        PlaySound(conquered_sound);
    }
    ...
    // DrawCircleV(game_zone_iba,50.0f,GREEN);
