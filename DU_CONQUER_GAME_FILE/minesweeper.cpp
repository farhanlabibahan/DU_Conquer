...
void draw_minesweeper() {
    ...
    for (int y = 0; y < GRID_HEIGHT; y++) {
        for (int x = 0; x < GRID_WIDTH; x++) {
            ...
            DrawRectangleLinesEx(rect, 1, Fade(WHITE, 0.05f));  // Very faint grid for debug
            Tile tile = grid[y][x];
            ...
            } else if (debugShowMines && tile.hasMine) {
                DrawRectangleLinesEx(rect, 2, RED);
            }
        }
    }

    DrawText(TextFormat("Flags Left: %d", flagsLeft), 20, 10, 20, YELLOW);
    DrawText(TextFormat("Time: %.1fs", gameTimer), 400, 10, 20, LIGHTGRAY);
