...
string game_rules_japanesse = "Let's see your drawing skills\n Don't derail from the lines";
...
void init_japanesse() {
    ...
    scale = (float)GetScreenHeight() / bg_image_japanesse.height;
    float y_pos_floor = (float)GetScreenHeight() - scale * 350;
   // playerPos_japanesse = (Vector2){-10, y_pos_floor};
    playerPos_japanesse = {2200*scale,GetScreenHeight()-400*scale};
    exit_zone_japanesse = {2200*scale,GetScreenHeight()-400*scale};
    game_zone_japanesse = {1950*scale,GetScreenHeight()-400*scale};
...
void logic_draw_japanesse() {
    ...
    if (!japanesse_game) pop_up_japanesse = "MANGA OVER ANYTHING ANYDAY";
