...
string game_rules_genetics = "Wait up!! Is something cooking!";
...
void init_genetics() {
    ...
    scale = (float)GetScreenHeight() / bg_image_genetics.height;
    playerPos_genetics = {4700*scale,screenHeight-400*scale};
    exit_zone_genetics = {4700*scale,screenHeight-400*scale};
    game_zone_genetics = {3120*scale,screenHeight-550*scale};
...
void logic_draw_genetics() {
    ...
    if (!genetics_game) pop_up_genetics = "Are the mismatched";
