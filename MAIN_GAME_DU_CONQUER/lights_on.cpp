...
void UpdateLightsOn() {
    ...
    if (!eee_game_2 && IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
    ...
    if (gameStarted && !eee_game_2) elapsedTime += GetFrameTime();
    if (!eee_game_2 && checkWin()) {
        eee_game_2 = true;
        lightsOnActive = false; 
        lightsOnBlocked = true;
        blockMessageTimer = 3.0f; // Show block message for 3 seconds
    }
    if (eee_game_2) flashTimer += GetFrameTime();
...
void DrawLightsOn() {
    ...
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
            ...
            if (grid[i][j] && eee_game_2 && ((int)(flashTimer * 5) % 2 == 0)) {
