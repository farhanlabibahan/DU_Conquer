...
string game_rules_philosophy = "Lights On Game Rules:\nTurn on all the lights to win.\nPress X to exit the game.";
...
void init_philosophy() {
    ...
    scale = (float)GetMonitorHeight(0) / bg_image_philosophy.height;
    float y_pos_floor = (float)GetMonitorHeight(0) - scale * 350;
   // playerPos_philosophy = (Vector2){-10, y_pos_floor};
    playerPos_philosophy = {2200*scale,GetMonitorHeight(0)-400*scale};
    exit_zone_philosophy = {2200*scale,GetMonitorHeight(0)-400*scale};
    game_zone_philosophy = {1950*scale,GetMonitorHeight(0)-400*scale};
...
void logic_draw_philosophy() {
    ...
    if (!philosophy_game) pop_up_philosophy = "Find and Solve the Clue";
